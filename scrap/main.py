...
BASE_SLUG = "المكتبة-القانونية/صيغ-العقود-و-الدعاوي/صيغ-العقود/المدنية"

DEFAULT_CATEGORIES = {
    # ── Main civil categories ───────────────────────────────────────────────
    "المدنية/الايجار":                         f"{BASE_SLUG}/الإيجار",
    "المدنية/الاعارة":                         f"{BASE_SLUG}/الإعارة",
    "المدنية/الحراسة":                         f"{BASE_SLUG}/الحراسة",
    "المدنية/الدخل الدائم":                    f"{BASE_SLUG}/الدخل-الدائم",
    "المدنية/الصلح":                           f"{BASE_SLUG}/الصلح",
    "المدنية/القرض":                           f"{BASE_SLUG}/القرض",
    "المدنية/القسمة والملكية الشائعة":         f"{BASE_SLUG}/القسمة-والملكية-الشائعة",
    "المدنية/الكفالة":                         f"{BASE_SLUG}/الكفالة",
    "المدنية/المرتب مدى الحياة":               f"{BASE_SLUG}/المرتب-مدى-الحياة",
    "المدنية/المقايضة":                        f"{BASE_SLUG}/المقايضة",
    "المدنية/الهبة":                           f"{BASE_SLUG}/الهبة",
    "المدنية/الوديعة":                         f"{BASE_SLUG}/الوديعة",
    "المدنية/الوكالة":                         f"{BASE_SLUG}/الوكالة",
    "المدنية/حوالة حق والدين":                 f"{BASE_SLUG}/حوالة-الحق-والدين",
    "المدنية/عقد استثمار":                     f"{BASE_SLUG}/عقد-استثمار",
    "المدنية/عقد بيع والوعود به":              f"{BASE_SLUG}/عقد-البيع-والوعد-به",
    # ── عقد بيع والوعود به sub-categories ───────────────────────────────────
    "المدنية/عقد بيع والوعود به/انذار البيوع":             f"{BASE_SLUG}/عقد-البيع-والوعد-به/إنذارات-البيوع",
    "المدنية/عقد بيع والوعود به/البيوع العقارية":          f"{BASE_SLUG}/عقد-البيع-والوعد-به/البيوع-العقارية",
    "المدنية/عقد بيع والوعود به/الملكية الفكرية واﻷدبية": f"{BASE_SLUG}/عقد-البيع-والوعد-به/الملكية-الفكرية-والأدبية",
    "المدنية/عقد بيع والوعود به/الوعد البيع":             f"{BASE_SLUG}/عقد-البيع-والوعد-به/الوعد-بالبيع",
    "المدنية/عقد بيع والوعود به/بيع التركة":               f"{BASE_SLUG}/عقد-البيع-والوعد-به/بيع-التركة",
    "المدنية/عقد بيع والوعود به/بيع الحقوق المتنازل عنها": f"{BASE_SLUG}/عقد-البيع-والوعد-به/بيع-الحقوق-المتنازع-عليها",
    "المدنية/عقد بيع والوعود به/بيع المنقولات":            f"{BASE_SLUG}/عقد-البيع-والوعد-به/بيع-المنقولات",
    "المدنية/عقود المقاولات وتعهدات البناء":               f"{BASE_SLUG}/عقود-المقاولات-وتعهدات-البناء",
}
...
CATEGORY_ALIASES = {
    "انذار البيوع": ["إنذارات البيوع", "إنذار البيوع", "انذارات البيوع"],
    "حوالة حق والدين": ["حوالة الحق والدين"],
    "عقد استثمار": ["عقد الاستثمار"],
    "عقد بيع والوعود به": ["عقد البيع والوعد به", "عقد البيع والوعود به"],
    "الملكية الفكرية واﻷدبية": ["الملكية الفكرية والأدبية", "الملكية الفكرية والادبية"],
    "الوعد البيع": ["الوعد بالبيع", "الوعد للبيع"],
    "الدخل الدائم": ["الدخل-الدائم"],
    "القسمة والملكية الشائعة": ["القسمة-والملكية-الشائعة"],
    "المرتب مدى الحياة": ["المرتب-مدى-الحياة"],
    "عقود المقاولات وتعهدات البناء": ["عقود مقاولات وتعهدات البناء", "عقود المقاولات وتعهدات البناء"],
}
...
def warm_up_session(session: requests.Session):
    ...
    try:
        ...
        log.info(f"   Homepage: {r.status_code} | Cookies: {list(session.cookies.keys())}")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if r.status_code == 200:
                ...
                log.warning(f"  ⛔ Blocked ({r.status_code}) — backing off {BLOCK_WAIT:.0f}s and stopping")
                time.sleep(BLOCK_WAIT)
                return None
            elif r.status_code in (500, 502, 503):
                log.warning(f"  ⚠️  Server error ({r.status_code}), waiting {RETRY_WAIT}s...")
...
def pick_test_category(paths: list[str]) -> str:
    target = normalize_arabic("الايجار")
    for path_item in paths:
        leaf = path_item.split("/")[-1]
        if normalize_arabic(leaf) == target:
            return path_item
    return "المدنية/الايجار"
...
def render_list(list_tag: Tag, ordered: bool, indent: int = 0) -> str:
    ...
    for li in list_tag.find_all("li", recursive=False):
        ...
        for child in li.find_all(["ul", "ol"], recursive=False):
            lines.append(render_list(child, ordered=(child.name == "ol"), indent=indent + 1))
...
def scrape_category(
    ...
    while True:
        ...
        if r is None:
            log.warning(f"  ⚠️  Could not fetch category page {page_num} — stopping this category")
            break
        ...
        log.info(f"  🔗 Found {len(article_links)} article(s) on page {page_num}")
        ...
        for idx, link in enumerate(article_links):
            ...
            log.info(f"  [{idx+1}/{len(article_links)}] Fetching: {art_title[:55]}")
            ...
            if not data["markdown"] or data["char_count"] < 150:
                log.warning(f"  ⚠️  Content too short ({data['char_count']} chars): {art_title[:50]}")
                stats["empty"] += 1
                continue

            # Save
            saved_path = save_contract(data, folder)
            log.info(f"  ✅ Saved [{data['char_count']} chars]: {saved_path.name}")
            stats["saved"] += 1
...
def main():
    ...
    parser.add_argument("--test",   action="store_true", help="Test mode: scrape only الايجار, first page only")
    parser.add_argument("--resume", action="store_true", help="Skip already downloaded contracts")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_DIR), help="Output directory")
    parser.add_argument("--categories-file", help="Path to category classification file")
    ...
    log.info(f"  Mode    : {'TEST' if args.test else 'FULL'} {'+ RESUME' if args.resume else ''}")
    ...
    category_index = fetch_category_index(session)
    if not category_index:
        log.warning("  ⚠️  Could not build category index — falling back to slug guesses")

    categories_file = find_categories_file(args.categories_file)
    category_paths: list[str] = []
    if categories_file:
        log.info(f"  Categories file: {categories_file}")
        category_paths = load_category_paths(categories_file)

    if args.test:
        if category_paths:
            test_path = pick_test_category(category_paths)
            test_leaf = test_path.split("/")[-1]
            cats = {test_path: resolve_category_slug(test_leaf, category_index)}
        else:
            cats = {"المدنية/الايجار": DEFAULT_CATEGORIES.get("المدنية/الايجار")}
        log.info("  Categories: 1 (test mode)\n")
    else:
        if categories_file and category_paths:
            cats = build_category_map(category_paths, category_index)
            log.info(f"  Categories: {len(cats)}\n")
        else:
            if categories_file and not category_paths:
                log.warning(f"  ⚠️  No categories found in: {categories_file}; using default map")
            else:
                log.warning("  ⚠️  No categories file found; using default map")
            default_paths = list(DEFAULT_CATEGORIES.keys())
            cats = build_category_map(default_paths, category_index) if category_index else DEFAULT_CATEGORIES
            log.info(f"  Categories: {len(cats)} (default map)\n")
